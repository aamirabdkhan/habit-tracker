...
ct.addEventListener("click",function(e){
    ...
    if(a==="sah"){var inp7=document.getElementById("snh"),nm6=inp7.value.trim();if(!nm6){toast("Enter a name");return}var d9=gDef();var exists=d9.habits.find(function(x){return (typeof x === "string" ? x : x.n) === nm6});if(exists){toast("Already exists");return}d9.habits.push({n:nm6,s:true,c:"rings"});sDef(d9);inp7.value="";render();var nextInp=document.getElementById("snh");if(nextInp)nextInp.focus();toast("Added");return}
    if(a==="srh"){e.stopPropagation();var i2=+t.dataset.i;var d10=gDef();d10.habits.splice(i2,1);sDef(d10);render();toast("Removed");return}
    if(a==="sae"){var inp8=document.getElementById("sne"),nm7=inp8.value.trim();if(!nm7){toast("Enter a name");return}var d11=gDef();if(d11.ex.find(function(x){return x.n===nm7})){toast("Already exists");return}d11.ex.push({n:nm7,s:false,c:"rings"});sDef(d11);inp8.value="";render();var nextInp=document.getElementById("sne");if(nextInp)nextInp.focus();toast("Added");return}
    ...
    if(a==="sahl"){var inp9=document.getElementById("snhl"),nm8=inp9.value.trim();if(!nm8){toast("Enter a name");return}var d14=gDef();if(d14.hl.find(function(x){return x.n===nm8})){toast("Already exists");return}d14.hl.push({n:nm8,s:false,c:"rings"});sDef(d14);inp9.value="";render();var nextInp=document.getElementById("snhl");if(nextInp)nextInp.focus();toast("Added");return}
    ...
    if(a==="sar"){var inp10=document.getElementById("snr"),nm9=inp10.value.trim();if(!nm9){toast("Enter a name");return}var d17=gDef();if(d17.rd.indexOf(nm9)!==-1){toast("Already exists");return}d17.rd.push(nm9);sDef(d17);inp10.value="";render();var nextInp=document.getElementById("snr");if(nextInp)nextInp.focus();toast("Added");return}
